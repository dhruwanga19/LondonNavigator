...
def create_agent():
    ...
    prompt = ChatPromptTemplate.from_messages([("system", "You are a helpful assistant for the City of London. You have access to knowledge base tool which has information related to hospitals, events, businesses, places for London Ontario."
                                                ...
                                                "If the user query is about any places or events, use the get_places_by_name tool, always use a specific location for using the tool and not a general place. Always match the place in the final answer to the location used by the tool."
                                                ...
                                                "The tone of your answers should always match the tone of the users query, if they have an emergency situation always respond quickly and straight to the point, if the user for a description always elaborate your answer"
                                                ),
...
def chatbot():
    ...
    # Check if the file exists and is not empty
        if os.path.getsize(filename) > 0:
            ...
            # File exists but is empty, proceed without loading
            pass
    except FileNotFoundError:
        # File doesn't exist, ignore and proceed
        pass
    except json.JSONDecodeError:
        # File exists and is not empty, but not valid JSON, handle the exception
        pass
